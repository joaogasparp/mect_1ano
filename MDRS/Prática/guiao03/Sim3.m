...
EventList = [ARRIVAL_DATA, tmp, GeneratePacketSize(), tmp];

for i = 1:n
    tmp = Clock + (16 + (24-16)*rand())/1000;
    EventList = [EventList; ARRIVAL_VOIP, tmp, 110 + (130-110)*rand(), tmp];
end
...
while TRANSPACKETS_DATA + TRANSPACKETS_VOIP < P  % Stopping criterium
    ...
    switch Event
        case ARRIVAL_DATA  % If first event is an ARRIVAL_DATA
            ...
            tmp = Clock + exprnd(1/lambda);
            EventList = [EventList; ARRIVAL_DATA, tmp, GeneratePacketSize(), tmp];
            if STATE == 0
                STATE = 1;
