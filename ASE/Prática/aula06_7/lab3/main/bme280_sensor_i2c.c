...
void bme280_read_data(i2c_master_dev_handle_t sensorHandle, bme280_data_t *data)
{
    ...
    // Solicita os dados de pressão, temperatura e umidade
    ESP_ERROR_CHECK(i2c_master_transmit_receive(sensorHandle, &reg, sizeof(reg), rawData, sizeof(rawData), -1));

    // Converte os dados brutos para valores reais (simplificado)
    ...
    // Conversão para valores reais (simplificado, sem calibração)
    data->temperature = adc_T / 100.0;
