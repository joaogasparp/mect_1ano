package sd.producerConsumer.method5;

import genclass.GenericIO;

/**
 *    General description:
 *       producers / consumers problem - monitor-based solution using basic building blocks from the Java concurrency
 *         library.
 */
...
public class ProducerConsumer
{
   ...
   */
   ...
   public static void main (String [] args)
   {
     ...
     GenericIO.writelnString ("Main thread ID is " + Thread.currentThread ().threadId ());
     ...
     { prod[i] = new Producer ("Producer_" + i, i, niter, fifo);
       GenericIO.writelnString ("Producer " + i + " thread ID " + " is " + prod[i].threadId ());
       GenericIO.writelnString ("Producer " + i + " thread name " + " is " + prod[i].getName ());
     }

     for (int i = 0; i < nCons; i++)
     { cons[i] = new Consumer ("Consumer_" + i, i, niter, fifo);
       GenericIO.writelnString ("Consumer " + i + " thread ID " + " is " + cons[i].threadId ());
