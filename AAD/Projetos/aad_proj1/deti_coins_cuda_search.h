...
static void deti_coins_cuda_search(u32_t n_random_words)
{
    ...
    coin_t coin[32];
    u32_t hash[32][4u];
    static u32_t interleaved_test_data[13u * 32u] __attribute__((aligned(16)));
    static u32_t interleaved_test_hash[4u * 32u] __attribute__((aligned(16)));
    u32_t v1 = 0x20202020, v2 = 0x20202020;

    initialize_cuda(0, "deti_coins_cuda_kernel_search.cubin", "deti_coins_cuda_kernel_search", 13u * 32u, 4u * 32u);

    for (lane = 0; lane < 32; lane++)
    {
        snprintf(coin[lane].coin_as_chars, 53, "DETI coin %c%c%39s\n", '0' + lane, '0' + lane, "");
    }

    for (n_attempts = n_coins = 0ul; stop_request == 0; n_attempts += 32)
    {
        for (lane = 0u; lane < 32u; lane++)
            for (idx = 0u; idx < 13u; idx++)
                interleaved_test_data[32u * idx + lane] = coin[lane].coin_as_ints[idx];

        md5_cuda(interleaved_test_data, interleaved_test_hash, 32u);

        for (lane = 0u; lane < 32u; lane++)
        {
            for (idx = 0u; idx < 4u; idx++)
                hash[lane][idx] = interleaved_test_hash[32u * idx + lane];
        ...
        for (lane = 0u; lane < 32u; lane++)
        {
            for (idx = 10u; idx < 13u * 32u - 1u && coin[lane].coin_as_chars[idx] == (u08_t)126; idx++)
