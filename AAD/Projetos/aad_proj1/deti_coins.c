...
#endif
#include "deti_coins_cpu_avx_search_server.h"
#include "deti_coins_cpu_avx_search_client.h"
#include "deti_coins_web_assembly.h"
#include "deti_coins_cpu_avx_search_opencl.h"
//
// main program
//
...
int main(int argc, char **argv)
{
  ...
  if ((argc >= 2 && argc <= 4) && argv[1][0] == '-' && argv[1][1] == 's')
  {
    ...
    if (seconds < 60u)
      seconds = 60u; // at least 1 minutes
    if (seconds > 7200u)
    ...
    case '4':
      ...
      break;
#endif
#ifdef DETI_COINS_CPU_AVX_SEARCH_SERVER
    case '5':
      printf("running server and clients for deti_coins_cpu_avx_search()...\n");
      ...
      if (pid == 0) {
          ...
          for (int i = 0; i < n_clients; i++) {
              ...
              if (client_pid == 0) {
                  ...
                  start_client(server_ip, seconds);
                  exit(0);
              }
          ...
          printf("server and clients have completed execution in %u seconds using deti_coins_cpu_avx_search().\n", seconds);
      }
      break;
#endif
#ifdef DETI_COINS_WEB_ASSEMBLY
    case '6':
      printf("searching for %u seconds using deti_coins_web_assembly()\n", seconds);
      fflush(stdout);
      deti_coins_web_assembly();
      break;
#endif
...
#endif
#ifdef DETI_COINS_CPU_AVX_SEARCH_SERVER
  fprintf(stderr, "       %s -s5 [seconds] [ignored]          # run server and clients to search for DETI coins using md5_cpu_avx\n", argv[0]);
#endif
#ifdef DETI_COINS_WEB_ASSEMBLY
  fprintf(stderr, "       %s -s6 [seconds] [ignored]          # search for DETI coins using web assembly\n", argv[0]);
#endif
#ifdef DETI_COINS_OPENCL_SEARCH
  fprintf(stderr, "       %s -s7 [seconds] [n_random_words]   # search for DETI coins using openCL \n", argv[0]);
#endif
#ifdef DETI_COINS_CPU_SPECIAL_SEARCH
