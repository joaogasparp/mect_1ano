...
re_step = (c_re_max - c_re_min) / n_samples
im_step = (c_im_max - c_im_min) / n_samples

for re_idx in range(n_samples):
    c_re = c_re_min + re_idx * re_step
    for im_idx in range(n_samples):
        c_im = c_im_min + im_idx * im_step
        z_re = 0.0
        z_im = 0.0
        n_iter = 0
        while n_iter < 256 and (z_re_sq := z_re * z_re) + (z_im_sq := z_im * z_im) <= 4.0:
            z_im = 2.0 * z_re * z_im + c_im
